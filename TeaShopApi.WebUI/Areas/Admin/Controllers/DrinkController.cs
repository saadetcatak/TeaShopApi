...
namespace TeaShopApi.WebUI.Areas.Admin.Controllers
{
    ...
    public class DrinkController : Controller
    {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync("https://localhost:7181/api/Drinks?id=" + id);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
        ...
        public async Task<IActionResult> UpdateDrink(UpdateDrinkDto updateDrinkDto)
        {
            ...
            var responseMessage = await client.PutAsync("https://localhost:7181/api/Drinks/", stringContent);
            if (responseMessage.IsSuccessStatusCode) 
            {
                return RedirectToAction("Index");
            }
