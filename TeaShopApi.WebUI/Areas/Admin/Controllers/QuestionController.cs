...
namespace TeaShopApi.WebUI.Areas.Admin.Controllers
{
    ...
    public class QuestionController : Controller
    {
        ...
        public async Task<IActionResult> CreateQuestion(CreateQuestionDto createQuestionDto)
        {
            ...
            var responseMessage=await client.PostAsync("https://localhost:7181/api/Question",content);
            if(responseMessage.IsSuccessStatusCode) 
            {
        ...
        {
            var client=_httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync("https://localhost:7181/api/Question?id=" + id);
            if(responseMessage.IsSuccessStatusCode) 
            {
                return RedirectToAction("Index");
            }
        ...
        public async Task<IActionResult> UpdateQuestion(UpdateQuestionDto updateQuestionDto)
        {
            ...
            StringContent content = new StringContent(jsonData,Encoding.UTF8,"application/json");
            var responseMessage = await client.PutAsync("https://localhost:7181/api/Question/", content);
            if(responseMessage.IsSuccessStatusCode) 
            {
                return RedirectToAction("Index");
            }
